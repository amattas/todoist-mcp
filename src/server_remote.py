...
if api_key:
    ...
    # Calculate MD5 hash of API key for dual-factor authentication
    api_key_hash = hashlib.md5(api_key.encode()).hexdigest()

    logger.info(f"API Key (first 8 chars): {api_key[:8]}...")
    logger.info(f"MD5 Hash: {api_key_hash}")
    ...
    async def not_found_handler(request: Request, exc: HTTPException):
        logger.warning(f"Access attempt to undefined route: {request.url.path}")
        return JSONResponse(
            status_code=404,
